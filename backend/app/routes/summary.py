...
def season_summary(
    ...
    try:
        result = generate_season_summary(db, field_name, season, current_user.id, generate_ai)
...
def field_summary(
    ...
    try:
        result = generate_field_summary(db, field_name, current_user.id, generate_ai)
        return SummaryResponse(**result)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
def crop_summary(
    ...
    try:
        result = generate_crop_summary(db, crop_name, current_user.id, generate_ai)
        return SummaryResponse(**result)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
