...
def add_record(
    ...
    text_to_embed = (
        f"Field: {record.field_name}, Crop: {record.crop_name}, "
        f"Season: {record.season}, Date: {record.date}, "
        f"Type: {record.record_type}, Details: {non_null_details}"
    )
...
def get_fields(
    ...
    fields = (
        db.query(FarmRecord.field_name)
        .filter(FarmRecord.user_id == current_user.id)
        .distinct()
        .all()
    )
...
def get_filtered_records(
    ...
    if season and season.strip() and season not in ("All Seasons", "All"):
    ...
    return q.order_by(FarmRecord.date.desc()).all()
